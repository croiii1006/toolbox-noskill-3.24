...
这份文档不讲业务流程，只讲 ORANGEN 这页的视觉语言。目标是把这页的 UI 设定拆成可以复刻的设计规范，包括字体、图标、颜色、圆角、边框、阴影、间距、布局、状态和交互气质。

## 1. 页面气质

ORANGEN 不是通用后台，也不是营销落地页。它的视觉定位更接近：
...
如果后面要在别的页面里 1:1 复刻 ORANGEN 的视觉，不是照搬某个组件，而是要同时满足这几条：
...
ORANGEN 的视觉不是“某个组件好看”，而是一整套克制的浅色 AI 工作台语言：
